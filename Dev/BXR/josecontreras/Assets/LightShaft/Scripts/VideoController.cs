...
public class VideoController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (sourceVideo.GetComponent<HighQualityPlayback>().videoQuality == HighQualityPlayback.VideoQuality.SD360 || sourceVideo.GetComponent<HighQualityPlayback>().videoQuality == HighQualityPlayback.VideoQuality.SD480)
        {
        ...
        {
            Debug.Log("Backward sync is a workaround for mobiles, you can disable this if is not using in mobile, unity video player there's a bug when you seek video to a backward time, that takes too long to load, so, we made a temporary workaround for this.");
        }
    }
    ...
    public bool useBackwardSyncImprovementWorkaround = false;
