...
namespace BrandXR
{
    public class bxrTweenInt: bxrTween
    {
        ...
        //--------------------------------------------//
        {
            ...
            if( info != null )
            {
                ...
                var cast = info.GetValue( uObject ) as int?;

                if( cast != null )
                {
                    if( showDebug ) { Debug.Log( "bxrTweenInt.cs GetStartValue() Found " + fieldName + ", value = " + (int)cast + " within Object fields" ); }
                ...
                {
                    if( showDebug ) { Debug.Log( "bxrTweenInt.cs GetStartValue() we found " + fieldName + ", but were unable to cast it to the type needed" ); }
                }
            }
            else
            {
                if( showDebug ) { Debug.Log( "bxrTweenInt.cs GetStartValue() unable to find any fields with the fieldName = " + fieldName + ", about to begin searching properties instead" ); }
            }
            ...
            foreach( PropertyInfo pInfo in pInfos )
            {
                ...
                if( pInfo.Name == fieldName && pInfo.CanRead && pInfo.CanWrite )
                {
                    ...
                    var cast = pInfo.GetValue( uObject, null ) as int?; //Use the '?' at the end of the value type to allow primitives and other 'non-nullables' to also have a 'null' value type

                    if( cast != null )
                    {
                        if( showDebug ) { Debug.Log( "Found " + fieldName + ", value = " + (int)cast + " within Object properties" ); }
                        propertyInfo = pInfo;
        ...
        //--------------------------------------------//
        {
            ...
            {
                //Debug.Log( "checkThis.GetType().Name = " + ( checkThis.GetType().IsSubclassOf( type ) ) + ", type.Name = " + type );
        ...
        //-------------------------------------------//
        public int GetDefaultStartValue<T>( T component, string fieldName )
        //-------------------------------------------//
        {
        ...
        //--------------------------------------------//
        public void Play<T>( T tweenThis, string fieldName, int startValue, int endValue, AnimationCurve easeCurve, float length, float delay, bool loop, UnityEvent onCompleteOrLoop ) where T : UnityEngine.Object
            //--------------------------------------------//
        {
            ...
            {
                if( showDebug ) { Debug.Log( "bxrTweenInt.cs Play() Unable to Play Tween, tweenThis variable is null or of the incorrect type" ); }
            }
